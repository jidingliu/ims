...
jQuery.validator.addMethod("idCard",function(value,element) {
	var idCard = /^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}[\d{1}|x]$/;
	return this.optional(element) || (idCard.test(value));
},"请输入正确的身份证号");
/*jQuery.validator.addMethod("accountUnique",function(value,element) {
	var returnResult = true;
	$.ajax({
		url : "userAccountCheck.action",
		data : {
			"user.account" : value
		},
		async : false,
...
jQuery.validator.addMethod("studentNumberUnique",function(value,element,param) {
	if(value==param) {
		return true;
	}
	var returnResult = true;
	$.ajax({
		url : "userStudentNumberCheck.action",
		data : {
...
jQuery.validator.addMethod("gradeNameUnique",function(value,element,param) {
	if(value==param) {
		return true;
	}
	var returnResult = true;
	$.ajax({
		url : "isExistGradeName.action",
		data : {
			"grade.name" : value
		},
		async : false,
...
jQuery.validator.addMethod("uniqueCheck",function(value,element,param) {
	if(value==param.paramValue) {
		return true;
	}
	var returnResult = true;
	var postData = {};
	postData[param.paramName] = value;
	$.ajax({
		url : param.checkUrl,
		data : postData,
		async : false,
